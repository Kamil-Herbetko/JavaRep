...
public class CustomDecorationWindowProc implements WinUser.WindowProc {
    ...
    final User32Calc INSTANCEEx;
    WinDef.HWND hwnd = new WinDef.HWND();
    BaseTSD.LONG_PTR defWndProc;

    public CustomDecorationWindowProc() {
        INSTANCEEx = (User32Calc) Native.load("user32", User32Calc.class, W32APIOptions.DEFAULT_OPTIONS);
    }

    public void init(WinDef.HWND hwnd) {
        this.hwnd = hwnd;
        if(is64Bit())
            defWndProc=INSTANCEEx.SetWindowLongPtr(hwnd, User32Calc.GWLP_WNDPROC, this);
        else
            defWndProc=INSTANCEEx.SetWindowLongPtr(hwnd, User32Calc.GWLP_WNDPROC, this);
    ...
    public LRESULT callback(HWND hwnd, int uMsg, WPARAM wparam, LPARAM lparam) {
        ...
        switch (uMsg) {
            ...
            case WM_DESTROY:
                if(is64Bit())
                    INSTANCEEx.SetWindowLongPtr(hwnd, User32Calc.GWLP_WNDPROC, defWndProc);
                else
                    INSTANCEEx.SetWindowLongPtr(hwnd, User32Calc.GWLP_WNDPROC, defWndProc);
                return new LRESULT(0);
            default:
